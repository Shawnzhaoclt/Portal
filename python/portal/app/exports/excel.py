...
def build_portal_excel_workbook(
    *,
    report_title: str,
    ...
    """Build a multi-sheet workbook with the same Portal export styling."""
    ...
    workbook.properties.title = report_title
    workbook.properties.subject = "Portal data export"
    workbook.properties.description = f"{report_title} multi-sheet export."

    grid_side = Side(style="thin", color=PORTAL_BORDER)
    grid_border = Border(left=grid_side, right=grid_side, top=grid_side, bottom=grid_side)
    for definition in sheets:
        if not definition.columns:
            raise ValueError(f"Sheet {definition.name} requires at least one column.")
        rows = list(definition.rows)
        columns = list(definition.columns)
        sheet = workbook.create_sheet(_safe_sheet_name(definition.name))
        sheet.sheet_properties.tabColor = PORTAL_BLUE
        sheet.sheet_properties.pageSetUpPr = PageSetupProperties(fitToPage=True, autoPageBreaks=False)
        sheet.sheet_view.showGridLines = False
        sheet.sheet_view.zoomScale = 90
        column_count = len(columns)
        _style_merged_row(sheet, 1, column_count, value=definition.title, fill=PORTAL_BLUE, color=PORTAL_WHITE, size=16, bold=True, height=30)
        record_label = "record" if len(rows) == 1 else "records"
        _style_merged_row(
            sheet,
            2,
            column_count,
            value=f"Generated: {generated.strftime('%Y-%m-%d %H:%M:%S')} | Exported by: {exported_by} | {len(rows):,} {record_label}",
            fill=PORTAL_WHITE,
            color=PORTAL_MUTED_TEXT,
            size=10,
            bold=False,
        ...
        _style_merged_row(sheet, 3, column_count, value=_metadata_filters(definition.filters), fill=PORTAL_PALE_BLUE, color=PORTAL_MUTED_TEXT, size=9, bold=False, height=28)
        for column_index, column in enumerate(columns, start=1):
            cell = sheet.cell(row=HEADER_ROW, column=column_index, value=column.heading)
            cell.font = Font(name="Aptos", size=10, bold=True, color=PORTAL_NAVY)
            cell.fill = PatternFill("solid", fgColor=PORTAL_LIGHT_BLUE)
        ...
        sheet.row_dimensions[HEADER_ROW].height = 30
        for row_index, record in enumerate(rows, start=HEADER_ROW + 1):
            fill = PatternFill("solid", fgColor=PORTAL_ALTERNATE_ROW if row_index % 2 == 1 else PORTAL_WHITE)
            for column_index, column in enumerate(columns, start=1):
                cell = sheet.cell(row=row_index, column=column_index, value=_cell_value(column, record))
                cell.font = Font(name="Aptos", size=10, color=PORTAL_TEXT)
                cell.fill = fill
                cell.border = grid_border
                cell.alignment = Alignment(vertical=vertical_alignment, wrap_text=column.wrap_text)
                if column.data_type == "datetime" and isinstance(cell.value, datetime):
                    cell.number_format = "m/d/yyyy h:mm:ss AM/PM"
                elif column.data_type == "date" and isinstance(cell.value, date):
                    cell.number_format = "m/d/yyyy"
