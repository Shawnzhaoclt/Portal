...
from portal.app.management.resource_ids import is_valid_resource_id, random_resource_id
...
def management_database_path() -> Path:
    configured = os.getenv("PORTAL_SYSTEM_DB", os.getenv("PORTAL_MANAGEMENT_DB", "")).strip()
    if configured:
        target = Path(configured)
        if not target.exists():
            seed = PORTAL_PACKAGE_ROOT / "data" / "portal_system.sqlite3"
            if not seed.is_file():
                seed = PORTAL_PACKAGE_ROOT / "data" / "portal_management.sqlite3"
            if seed.is_file() and seed.resolve() != target.resolve():
                target.parent.mkdir(parents=True, exist_ok=True)
                shutil.copy2(seed, target)
        return target
    system_database = PORTAL_PACKAGE_ROOT / "data" / "portal_system.sqlite3"
    return system_database if system_database.is_file() else PORTAL_PACKAGE_ROOT / "data" / "portal_management.sqlite3"
