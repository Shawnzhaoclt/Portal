...
from portal.app.management.models import Team, User
...
CRITICAL_TEAM_WORKORDER_SORT_EXPRESSIONS = {
    ...
    "facility_id": "PORTAL_INT(facility_id)",
    "submit_to": "submit_to",
...
def critical_team_connection() -> Iterator[sqlite3.Connection]:
    ...
    if source.source_type.lower() != "sqlite_snapshot":
        raise HTTPException(
            status_code=503,
            detail={
                "message": "Critical Team datasource must use the published SQLite snapshot.",
                "source_type": source.source_type,
            },
        )
    try:
        with portal_sources_connection() as connection:
            yield connection
    except sqlite3.Error as error:
        raise HTTPException(
            status_code=503,
            detail={
                "message": "Could not read the published Portal SQLite datasource.",
    ...
    return f"""
        WITH custom_fields AS (
            SELECT
                ...
                MAX(CASE WHEN CUSTFIELDID = 7 THEN PORTAL_DATE(NULLIF(TRIM(CUSTFIELDVALUE), '')) END) AS INSP_COMP_DATE,
                MAX(CASE WHEN CUSTFIELDID = 10 THEN PORTAL_DATE(NULLIF(TRIM(CUSTFIELDVALUE), '')) END) AS REPORT_COMP_DATE,
        ...
        critical_team_workorders AS (
            SELECT
                ...
                PORTAL_INT(cf.WORKORDERSID) AS workorders_id,
                ...
                PORTAL_DATETIME(wo.PROJSTARTDATE) AS project_start_date,
                PORTAL_DATETIME(wo.DATEWOCLOSED) AS wo_closed_date,
...
def fetch_all(cursor: sqlite3.Cursor, sql: str, params: list[Any] | None = None) -> tuple[list[str], list[Any]]:
    cursor.execute(sql, params or [])
    names = [description[0] for description in cursor.description]
    return names, cursor.fetchall()


def fetch_one(cursor: sqlite3.Cursor, sql: str, params: list[Any] | None = None) -> tuple[list[str], Any]:
    cursor.execute(sql, params or [])
    names = [description[0] for description in cursor.description]
...
def critical_team_year_clause(
    ...
    if numeric_years:
        ...
        clauses.append(
            f"CAST(strftime('%Y', {sql_identifier(date_column)}) AS INTEGER) IN ({placeholders})"
        )
...
def overview_date_predicate(date_column: str) -> str:
    date_expression = f"date({sql_identifier(date_column)})"
    return (
    ...
    with critical_team_connection() as con:
        ...
        for key, column in CRITICAL_TEAM_DATE_COLUMNS.items():
            _names, rows = fetch_all(
                ...
                f"""
                {critical_team_source_cte(source)}
                SELECT DISTINCT CAST(strftime('%Y', {sql_identifier(column)}) AS INTEGER) AS value
                FROM critical_team_workorders
...
def critical_team_sheet_data(
    ...
    with critical_team_connection() as con:
        ...
        month_start_expression = (
            ...
            f"ELSE date({sql_identifier(date_column)}, 'start of month') END"
        )
...
def critical_team_workorders(
    ...
    with critical_team_connection() as con:
        ...
        if sort_by == "condition_risk":
            ...
            names, rows = fetch_all(
                ...
                LIMIT ? OFFSET ?
                """,
                [*critical_team_base_params(source), *params, limit, offset],
            )
    ...
    for record in records:
        workorder_id = str(record.get("workorder_id") or "").strip()
        record["workorder_url"] = workorder_url_template.replace("{id}", workorder_id).replace("{value}", workorder_id) if workorder_id else ""
