...
LOCAL_RUNTIME_ROOT = Path(__file__).resolve().parent / "runtime"
SOURCE_RUNTIME_ROOT = Path(r"C:\Users\105692\scripts\stm_risk_models\map_tiles")


def default_runtime_root() -> Path:
    configured = os.environ.get("PORTAL_MAP_TILES_RUNTIME_ROOT", os.environ.get("MAP_TILES_RUNTIME_ROOT", "")).strip()
    if configured:
        return Path(configured).resolve()

    local_pmtiles = LOCAL_RUNTIME_ROOT / "build" / "pmtiles" / "planning_project.pmtiles"
    local_duckdb = LOCAL_RUNTIME_ROOT / "build" / "staging" / "duckdb" / "stm_risk.duckdb"
    if local_pmtiles.exists() and local_duckdb.exists():
        return LOCAL_RUNTIME_ROOT.resolve()

    if SOURCE_RUNTIME_ROOT.exists():
        return SOURCE_RUNTIME_ROOT.resolve()
...
INVENTORY_DUCKDB_PATH = Path(
    r"G:\Strategic Planning\Planning\stm_risk_data_opt_test\intermediate\inventory\inventory.db"
)
CITY_PIPES_DUCKDB_PATH = Path(
    r"G:\Strategic Planning\Planning\stm_risk_data\intermediate\citypipes\citypipes.db"
)
...
class BackendState:
    ...
    def maplibre_dir(self) -> Path:
        return self.project_config.paths.get("maplibre", self.project_root / "build" / "maplibre")

    @property
    def pmtiles_dir(self) -> Path:
        return self.project_config.paths.get("pmtiles", self.project_root / "build" / "pmtiles")

    @property
    def terrain_dir(self) -> Path:
        return self.project_config.paths.get("terrain", self.project_root / "build" / "terrain")

    @property
    def reports_dir(self) -> Path:
        return self.project_config.paths.get("reports", self.project_root / "build" / "reports")

    @property
    def duckdb_path(self) -> Path:
        return self.project_config.paths.get("duckdb", self.project_root / "build" / "staging" / "duckdb" / "stm_risk.duckdb")


def create_app() -> FastAPI:
    app = FastAPI(
        title="STM Risk Map Tiles API",
        description="Backend API for generated MapLibre styles, PMTiles, and project metadata.",
        version="0.1.0",
    )
    app.add_middleware(
        CORSMiddleware,
    ...
    def maplibre_style(style_path: str) -> FileResponse:
        state = get_state()
...
def get_state() -> BackendState:
    config_path = Path(os.environ.get("PORTAL_MAP_TILES_CONFIG", os.environ.get("MAP_TILES_CONFIG", DEFAULT_CONFIG))).resolve()
    try:
...
def cors_origins() -> list[str]:
    raw = os.environ.get("MAP_TILES_CORS_ORIGINS", "*").strip()
    if raw in {"", "*"}:
        return ["*"]
...
def query_asset_search(state: BackendState, query: str, limit: int) -> list[dict[str, Any]]:
    try:
        ...
        raise HTTPException(status_code=500, detail="The backend Python environment must include duckdb.") from exc
...
def open_sdw_connection() -> Any:
    try:
        ...
        raise HTTPException(status_code=500, detail="The backend Python environment must include pyodbc.") from exc

    configured = os.environ.get("MAP_TILES_SDW_CONNECTION", "").strip()
    if configured:
        try:
            return pyodbc.connect(configured, timeout=10)
        except Exception as exc:
            raise HTTPException(status_code=502, detail=f"Could not connect to SDW SQL Server: {exc}") from exc
    ...
    ordered = [driver for driver in preferred if driver in drivers] + [driver for driver in drivers if driver not in preferred]
    ...
    for driver in ordered:
        ...
        except Exception as exc:  # noqa: BLE001 - try all installed SQL Server drivers
            last_error = exc
    raise HTTPException(status_code=502, detail=f"Could not connect to SDW SQL Server: {last_error}")
...
def bbox_to_stateplane_wkt(bbox: list[float]) -> str:
    try:
        ...
        raise HTTPException(status_code=500, detail="The backend Python environment must include pyproj.") from exc

    west, south, east, north = bbox
    # Inventory geometries are stored in NAD83 / North Carolina StatePlane feet.
    # In the backend conda env, the WGS84 transformation path returns infinities,
...
def stateplane_wkb_to_wgs84_geojson(value: Any) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="The backend Python environment must include shapely.") from exc
