...
rem Registers a per-user Friday schedule for run-business-retention.bat.
rem It reads the configured HH:MM time from portal.settings.json.
...
set "TASK_NAME=StormWater Portal Business Retention"
...
for /f "usebackq delims=" %%A in (`"%PYTHON_EXE%" -c "import json,sys; print(json.load(open(sys.argv[1], encoding='utf-8-sig'))['maintenance']['businessRetention']['schedule']['time'])" "%PORTAL_SETTINGS%"`) do set "RUN_TIME=%%A"
if not defined RUN_TIME (
  echo ERROR: Could not read maintenance.businessRetention.schedule.time.
  exit /b 1
)

schtasks /Create /TN "%TASK_NAME%" /SC WEEKLY /D FRI /ST "%RUN_TIME%" /TR "cmd.exe /d /c \"%COORDINATOR_DIRECTORY%run-business-retention.bat\"" /F
if errorlevel 1 (
  echo ERROR: The weekly business-retention task was not registered.
  exit /b 1
)

echo Registered "%TASK_NAME%" for every Friday at %RUN_TIME%.
echo The task runs only when maintenance.businessRetention.automaticEnabled is true.
exit /b 0
