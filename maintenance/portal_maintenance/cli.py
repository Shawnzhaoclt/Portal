...
def _default_database(kind: str) -> Path:
    filename = "portal_system.sqlite3" if kind == "system" else "portal_business.sqlite3"
    return PROJECT_ROOT / "python" / "portal" / "data" / filename


def _default_backup_path(database_path: Path) -> Path:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    return PROJECT_ROOT / "maintenance" / "backups" / f"{database_path.stem}_{timestamp}.sqlite3"
...
def _database_path(value: str | None, kind: str) -> Path:
    return Path(value).expanduser().resolve() if value else _default_database(kind)


def _configure_database_env(database_path: Path, kind: str) -> None:
    database_path.parent.mkdir(parents=True, exist_ok=True)
    import os

    if kind == "system":
        os.environ["PORTAL_SYSTEM_DB"] = str(database_path)
        os.environ["PORTAL_MANAGEMENT_DB"] = str(database_path)
        os.environ.setdefault("PORTAL_BUSINESS_DB", str(_default_database("business")))
    else:
        os.environ["PORTAL_BUSINESS_DB"] = str(database_path)
        os.environ.setdefault("PORTAL_SYSTEM_DB", str(_default_database("system")))
        os.environ.setdefault("PORTAL_MANAGEMENT_DB", os.environ["PORTAL_SYSTEM_DB"])
...
def _print_summary(summary: dict[str, Any], *, json_output: bool) -> None:
    ...
    if not summary.get("exists"):
        ...
        return
    print(f"Portal database: {summary.get('database')}")
...
def _run_upgrade(database_path: Path, kind: str) -> None:
    _configure_database_env(database_path, kind)
    ...
    if kind == "system":
        ...
        from portal.app.resources.reports.proactive_team_cctv_review import ensure_report_schema

        ensure_report_schema()


def init_db(args: argparse.Namespace) -> int:
    database_path = _database_path(args.db, args.kind)
    _configure_database_env(database_path, args.kind)
    _configure_import_path()
    ...
    if args.json:
        ...
        print(f"Portal {args.kind} database deployed: {database_path}")
...
def upgrade_db(args: argparse.Namespace) -> int:
    database_path = _database_path(args.db, args.kind)
    _run_upgrade(database_path, args.kind)
    summary = _database_summary(database_path)
    if args.json:
        print(json.dumps(summary, indent=2))
    else:
        print(f"Portal {args.kind} database upgraded: {database_path}")
        _print_summary(summary, json_output=False)
    return 0
...
def _add_common_db_argument(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--kind", choices=("system", "business"), default="business", help="Database class. Defaults to business.")
    parser.add_argument("--db", help="SQLite DB path. Defaults to the source-tree database for --kind.")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Portal database maintenance utilities.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    init_parser = subparsers.add_parser("init-db", help="Deploy a system or business DB.")
    _add_common_db_argument(init_parser)
    init_parser.add_argument("--json", action="store_true", help="Print JSON summary.")
    init_parser.set_defaults(func=init_db)

    upgrade_parser = subparsers.add_parser("upgrade-db", help="Apply schema upgrades for a system or business DB.")
    ...
    backup_parser.add_argument("--backup", help="Backup file path. Defaults to maintenance/backups/<db>_<timestamp>.sqlite3.")
    ...
    summary_parser = subparsers.add_parser("summary", help="Print a system or business DB summary.")
