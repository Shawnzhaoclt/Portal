...
type TopRecordTab = 'culverts' | 'structures' | 'pipes'
...
const CLTEX_VECTOR_BASEMAP_STYLE_URL = clientSetting('externalServices', 'maps', 'cltexVectorBasemapStyleUrl')
const CLTEX_VECTOR_TILE_URL = clientSetting('externalServices', 'maps', 'cltexVectorTileUrl')
...
const NO_DATA_FILL_COLOR: [number, number, number, number] = [148, 163, 184, 70]
...
const RENDERER_RISK_ORDER = ['pipes:RISK', 'pipes:COND_RISK', 'pipes:FLOOD_RISK', 'pipes:CLOG_RISK', 'structures:COND_RISK', 'both:COND_RISK']
const RENDERER_MEASURE_ORDER = ['Max', 'Total', 'Avg', 'Median']
const TOP_RECORD_RISK_OPTIONS: Array<{
  ...
  },
  {
    ...
    label: 'Condition',
    fields: {
      culverts: 'CONDITION_RISK_PIPES_MAX',
      structures: 'COND_RISK',
      pipes: 'COND_RISK',
    },
  },
  {
    ...
    label: 'Flood',
    fields: {
      culverts: 'FLOOD_RISK_PIPES_MAX',
      structures: 'FLOOD_RISK',
      pipes: 'FLOOD_RISK',
    },
  },
  {
    ...
    label: 'Clog',
    fields: {
      culverts: 'CLOG_RISK_PIPES_MAX',
      structures: 'CLOG_RISK',
      pipes: 'CLOG_RISK',
    },
  },
]
const RENDERER_RAMP: Array<[number, number, number]> = [
  [34, 197, 94],
  [132, 204, 22],
  [245, 158, 11],
  [249, 115, 22],
...
function numberFromProperties(properties: Record<string, unknown> | undefined, field: string) {
  if (!properties) return null
  return numberFromProperty(properties[field])
}
...
function rendererRampColor(value: number | null, min: number, max: number, alpha: number): [number, number, number, number] {
  if (value === null || !Number.isFinite(value)) return alpha > 180 ? NO_DATA_LINE_COLOR : NO_DATA_FILL_COLOR
  if (!Number.isFinite(min) || !Number.isFinite(max) || max <= min) {
    const [r, g, b] = RENDERER_RAMP[0]
    return [r, g, b, alpha]
  }
  const ratio = Math.max(0, Math.min(1, (value - min) / (max - min)))
  const index = Math.min(RENDERER_RAMP.length - 1, Math.floor(ratio * RENDERER_RAMP.length))
  const [r, g, b] = RENDERER_RAMP[index]
  return [r, g, b, alpha]
}
...
function buildHistogram(values: number[], binCount = 20, domain?: { min: number; max: number } | null): HistogramData | null {
...
function formatRendererLabelValue(value: number | null) {
  return value === null ? '' : value.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 })
}

function rendererMetricLabel(renderer: GISLayerRenderer | null | undefined) {
  switch (renderer?.metric) {
    case 'COND_RISK':
...
function rendererRiskKey(renderer: GISLayerRenderer) {
  return `${renderer.source}:${renderer.metric}`
}

function rendererRiskTypeLabel(renderer: GISLayerRenderer | null | undefined) {
  if (!renderer) return 'Single Symbol'
  const key = rendererRiskKey(renderer)
  switch (key) {
    case 'pipes:RISK':
      return 'Total Risk - Pipes Only'
    case 'pipes:COND_RISK':
      return 'Condition Risk - Pipes Only'
    case 'pipes:FLOOD_RISK':
      return 'Flood Risk - Pipes Only'
    case 'pipes:CLOG_RISK':
      return 'Clog Risk - Pipes Only'
    case 'structures:COND_RISK':
      return 'Condition Risk - Structures Only'
    case 'both:COND_RISK':
      return 'Condition Risk - Pipes and Structures'
    default:
      return renderer.label
  }
}

function rendererRiskTypeDescription(renderer: GISLayerRenderer) {
  if (renderer.source === 'pipes') return 'Uses pipe asset inspection records only.'
  if (renderer.source === 'structures') return 'Uses structure inspection records only.'
  return 'Combines pipe and structure inspection records.'
}

function rendererMeasureLabel(renderer: GISLayerRenderer | null | undefined) {
  switch (renderer?.measure?.toLowerCase()) {
    case 'average':
      return 'Avg'
    case 'maximum':
    case 'max':
...
export default function GISDashboard() {
  ...
  const [rendererByLayer, setRendererByLayer] = useState<Record<string, string | null>>({})
  const [rendererWizardOpen, setRendererWizardOpen] = useState(false)
  const [wizardRiskKey, setWizardRiskKey] = useState('')
  const [wizardMeasure, setWizardMeasure] = useState('')
  const [topRecordTab, setTopRecordTab] = useState<TopRecordTab>('culverts')
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      setRendererByLayer((current) =>
        Object.fromEntries(
          metadata.layers.map((layer) => {
            const currentField = current[layer.id]
            const renderers = layer.renderers ?? []
            const validCurrentField = renderers.some((renderer) => renderer.field === currentField)
            const defaultField =
              layer.id === FACILITY_RENDERER_LAYER_ID && renderers.some((renderer) => renderer.field === DEFAULT_FACILITY_RENDERER_FIELD)
                ? DEFAULT_FACILITY_RENDERER_FIELD
                : renderers[0]?.field ?? null
            return [layer.id, validCurrentField ? currentField : defaultField]
          }),
        ),
      )
  ...
  async function loadSearchFeatures(nextLayers: GISLayerMeta[]) {
    try {
      const layerFeatures = await Promise.all(
        nextLayers.map(async (layer) => [layer.id, await fetchGISLayerFeatures(layer.id, { limit: 5000, bbox: null })] as const),
      )
  ...
  async function loadLayerFeatures(nextLayers: GISLayerMeta[], bbox: [number, number, number, number] | null, showLoading: boolean) {
    ...
    try {
      const layerFeatures = await Promise.all(
        nextLayers.map(async (layer) => [layer.id, await fetchGISLayerFeatures(layer.id, { limit: 5000, bbox })] as const),
      )
  ...
  const rendererStatsByLayer = useMemo(() => {
    return Object.fromEntries(
      layers.map((layer) => {
        const field = rendererByLayer[layer.id]
        const values =
          field && filteredFeatureData[layer.id]
            ? filteredFeatureData[layer.id].features
                .map((feature) => numberFromProperties(feature.properties, field))
  ...
  }, [filteredFeatureData, layers, rendererByLayer])
  ...
  const deckLayers = useMemo(
    () => {
      ...
      > = layers
        ...
        .map((layer) => {
          ...
          const rendererField = rendererByLayer[layer.id]
          const rendererStats = rendererStatsByLayer[layer.id]
          return new GeoJsonLayer({
            ...
            getFillColor: (rendererField && rendererStats
              ? ({ properties }: { properties?: Record<string, unknown> }) =>
                  rendererRampColor(numberFromProperties(properties, rendererField), rendererStats.min, rendererStats.max, isPoint ? 190 : 150)
              : hexToRgba(layer.color, isPoint ? 170 : 72)) as never,
            getLineColor: (rendererField && rendererStats
              ? ({ properties }: { properties?: Record<string, unknown> }) =>
                  rendererRampColor(numberFromProperties(properties, rendererField), rendererStats.min, rendererStats.max, 235)
              : color) as never,
            getTextColor: color,
            onClick: ({ object }) => {
      ...
      const facilityRendererField = rendererByLayer.facility_polygons
      const facilityRenderer = layers
        .find((layer) => layer.id === 'facility_polygons')
        ?.renderers?.find((renderer) => renderer.field === facilityRendererField)
      const facilityRiskLabel = rendererMetricLabel(facilityRenderer)
      const facilityMeasureLabel = rendererMeasureLabel(facilityRenderer)
      const showFacilityLabels =
        Boolean(visibleLayers.facility_polygons && facilityRendererField) && scaleInfo.denominator <= FACILITY_LABEL_MAX_SCALE_DENOMINATOR
      if (showFacilityLabels && facilityRendererField) {
        const labelData = (filteredFeatureData.facility_polygons?.features ?? [])
          .map((feature): FacilityLabelDatum | null => {
            const rendererValue = numberFromProperties(feature.properties, facilityRendererField)
            const placement = geometryLabelPlacement(feature.geometry, viewState.zoom)
            if (rendererValue === null || !placement) return null
            const text = `${facilityRiskLabel} (${facilityMeasureLabel}): ${formatRendererLabelValue(rendererValue)}\nFacility ID: ${formatValue(feature.properties.FacilityID)}`
            return {
    ...
    [filteredFeatureData, flashTarget, flashVisible, layers, mapFrameSize, rendererByLayer, rendererStatsByLayer, scaleInfo.denominator, viewState, visibleLayers],
  )
  ...
  const recordCountPair = (current: number, total: number): CountMetricValue => ({ current, total })
  ...
  const metricCards = [
    { label: 'Number of Culverts', value: recordCountPair(culvertFeatures.length, layerTotalCount(FACILITY_RENDERER_LAYER_ID)), detail: '', icon: Database },
    { label: 'Number of Pipes', value: recordCountPair(pipeFeatures.length, layerTotalCount('critical_asset_pipes')), detail: '', icon: Layers },
    { label: 'Number of Structures', value: recordCountPair(structureFeatures.length, layerTotalCount('critical_asset_structures')), detail: '', icon: Layers },
    { label: 'Max Culvert Risk', value: max(valuesFromFeatures(culvertFeatures, 'TOTAL_RISK_PIPES_MAX')), detail: 'Total risk - pipes', icon: Target },
    {
      label: 'Max Culvert Condition',
      value: max(valuesFromFeatures(culvertFeatures, 'CONDITION_RISK_BOTH_MAX')),
      detail: 'Condition risk - both',
      icon: Activity,
    },
    { label: 'Max Culvert Flood', value: max(valuesFromFeatures(culvertFeatures, 'FLOOD_RISK_PIPES_MAX')), detail: 'Flood risk - pipes', icon: BarChart3 },
    { label: 'Max Culvert Clog', value: max(valuesFromFeatures(culvertFeatures, 'CLOG_RISK_PIPES_MAX')), detail: 'Clog risk - pipes', icon: ShieldAlert },
    {
      label: 'Max Structure Condition',
      value: max(valuesFromFeatures(structureFeatures, 'COND_RISK')),
      detail: 'Condition risk - structures',
      icon: Activity,
    },
  ]
  const selectedEntries = featureDetailEntries(selectedFeature)
  const facilityLayer = useMemo(() => layers.find((layer) => layer.id === FACILITY_RENDERER_LAYER_ID), [layers])
  const facilityRenderers = facilityLayer?.renderers ?? []
  const activeFacilityRenderer = useMemo(
    () => facilityRenderers.find((renderer) => renderer.field === rendererByLayer[FACILITY_RENDERER_LAYER_ID]) ?? null,
    [facilityRenderers, rendererByLayer],
  )
  const activeFacilityRendererStats = rendererStatsByLayer[FACILITY_RENDERER_LAYER_ID]
  const activeFacilityHistogram = useMemo(() => {
    if (!activeFacilityRenderer) return null
    const domainValues = allCulvertFeatures
      .map((feature) => numberFromProperty(feature.properties[activeFacilityRenderer.field]))
      .filter((value): value is number => value !== null)
    if (!domainValues.length) return null
    const filteredValues = culvertFeatures
      .map((feature) => numberFromProperty(feature.properties[activeFacilityRenderer.field]))
      .filter((value): value is number => value !== null)
    return buildHistogram(filteredValues, 10, {
      min: Math.min(...domainValues),
      max: Math.max(...domainValues),
    })
  }, [activeFacilityRenderer, allCulvertFeatures, culvertFeatures])
  const wizardRiskOptions = useMemo(() => {
    const options = new globalThis.Map<string, GISLayerRenderer>()
    facilityRenderers.forEach((renderer) => {
      const key = rendererRiskKey(renderer)
      if (!options.has(key)) options.set(key, renderer)
    })
    return [...options.entries()].sort((first, second) => {
      const firstIndex = RENDERER_RISK_ORDER.indexOf(first[0])
      const secondIndex = RENDERER_RISK_ORDER.indexOf(second[0])
  ...
  const wizardMeasureOptions = useMemo(() => {
    const options = new globalThis.Map<string, GISLayerRenderer>()
    facilityRenderers
      .filter((renderer) => rendererRiskKey(renderer) === wizardRiskKey)
      .forEach((renderer) => {
        if (!options.has(renderer.measure)) options.set(renderer.measure, renderer)
      })
    return [...options.entries()].sort((first, second) => {
      const firstIndex = RENDERER_MEASURE_ORDER.indexOf(first[0])
      const secondIndex = RENDERER_MEASURE_ORDER.indexOf(second[0])
      return (firstIndex === -1 ? 999 : firstIndex) - (secondIndex === -1 ? 999 : secondIndex)
    })
  }, [facilityRenderers, wizardRiskKey])
  const wizardSelectedRenderer =
    facilityRenderers.find((renderer) => rendererRiskKey(renderer) === wizardRiskKey && renderer.measure === wizardMeasure) ?? null

  function openRendererWizard() {
    const startingRenderer = activeFacilityRenderer ?? facilityRenderers[0]
    setWizardRiskKey(startingRenderer ? rendererRiskKey(startingRenderer) : '')
    setWizardMeasure(startingRenderer?.measure ?? '')
    setRendererWizardOpen(true)
  }

  function selectWizardRisk(nextRiskKey: string) {
    const nextRenderers = facilityRenderers.filter((renderer) => rendererRiskKey(renderer) === nextRiskKey)
    const nextMeasure =
      nextRenderers.find((renderer) => renderer.measure === wizardMeasure)?.measure ??
      nextRenderers.find((renderer) => renderer.measure === activeFacilityRenderer?.measure)?.measure ??
      nextRenderers[0]?.measure ??
      ''
    setWizardRiskKey(nextRiskKey)
    setWizardMeasure(nextMeasure)
  }

  function applyRendererWizard() {
    if (!wizardSelectedRenderer) return
    setRendererByLayer((current) => ({ ...current, [FACILITY_RENDERER_LAYER_ID]: wizardSelectedRenderer.field }))
    setRendererWizardOpen(false)
  }
  ...
  const topRecordTabs = useMemo(() => {
    ...
    const riskOptionForTab = (tab: TopRecordTab) =>
      TOP_RECORD_RISK_OPTIONS.find((option) => option.key === topRecordRiskByTab[tab]) ?? TOP_RECORD_RISK_OPTIONS[0]
    const culvertRiskOption = riskOptionForTab('culverts')
    const structureRiskOption = riskOptionForTab('structures')
    const pipeRiskOption = riskOptionForTab('pipes')
    return {
      culverts: topLayerItems(
        FACILITY_RENDERER_LAYER_ID,
        culvertRiskOption.fields.culverts,
        'FacilityID',
        culvertRiskOption.label,
        'FacilityID',
      ),
      structures: topLayerItems('critical_asset_structures', structureRiskOption.fields.structures, 'FacilityID', structureRiskOption.label),
      pipes: topLayerItems('critical_asset_pipes', pipeRiskOption.fields.pipes, 'FacilityID', pipeRiskOption.label),
    } satisfies Record<TopRecordTab, TopRecordItem[]>
  }, [filteredFeatureData, layers, topRecordRiskByTab])

  const currentTopRecords = topRecordTabs[topRecordTab]
  const currentTopRecordRisk = topRecordRiskByTab[topRecordTab]
  ...
  return (
    <div className="gis-dashboard gis-control-room gis-facility-dashboard">
          ...
          <div className="gis-map-frame" ref={mapFrameRef}>
            ...
            {visibleLayers[FACILITY_RENDERER_LAYER_ID] && activeFacilityRenderer && activeFacilityHistogram ? (
              <div className="gis-culvert-histogram" aria-label={`Filtered Culvert histogram for ${rendererRiskTypeLabel(activeFacilityRenderer)}`}>
                <div className="gis-culvert-histogram-header">
                  <span>Culvert Risk Distribution</span>
                  <strong>{activeFacilityHistogram.total.toLocaleString()}</strong>
                </div>
                <small>
                  {rendererRiskTypeLabel(activeFacilityRenderer)} / {rendererMeasureLabel(activeFacilityRenderer)}
                </small>
                <div className="gis-culvert-histogram-bars">
                  {activeFacilityHistogram.bins.map((bin, index) => {
                    const [r, g, b] = RENDERER_RAMP[Math.min(RENDERER_RAMP.length - 1, Math.floor((index / Math.max(1, activeFacilityHistogram.bins.length - 1)) * (RENDERER_RAMP.length - 1)))]
                    const height = activeFacilityHistogram.maxCount ? Math.max(4, Math.round((bin.count / activeFacilityHistogram.maxCount) * 46)) : 4
                    return (
                      <div
            ...
            {visibleLayers[FACILITY_RENDERER_LAYER_ID] && activeFacilityRenderer && activeFacilityRendererStats ? (
              <div className="gis-culvert-legend" aria-label={`Culvert Facility legend for ${rendererRiskTypeLabel(activeFacilityRenderer)}`}>
                <div>
                  <span>Culvert Facility</span>
                  <strong>{rendererRiskTypeLabel(activeFacilityRenderer)}</strong>
                </div>
                <small>{rendererMeasureLabel(activeFacilityRenderer)} values</small>
                <i aria-hidden="true" />
                <div>
                  <span>Low {formatMetric(activeFacilityRendererStats.min)}</span>
                  <span>High {formatMetric(activeFacilityRendererStats.max)}</span>
                </div>
              </div>
            ) : null}
                ...
                <div className={`gis-metric-card ${lowerDetailLine ? 'gis-metric-card-lower-detail' : ''}`} key={metric.label}>
                  ...
                  <strong className={isCountMetricValue(metric.value) ? 'gis-record-count-value' : undefined}>
                    {isCountMetricValue(metric.value) ? (
                      <>
                        {metric.value.current.toLocaleString()}
                        <small>/ {metric.value.total.toLocaleString()}</small>
                      </>
      ...
      {rendererWizardOpen ? (
        <div className="gis-renderer-modal-backdrop" role="presentation" onClick={() => setRendererWizardOpen(false)}>
          <section
            aria-modal="true"
            className="gis-renderer-modal"
            role="dialog"
            aria-label="Facility renderer wizard"
            onClick={(event) => event.stopPropagation()}
          >
            <header className="gis-renderer-modal-header">
              <div>
                <span>Renderer Wizard</span>
                <strong>Culvert Facility Risk Display</strong>
              </div>
              <button type="button" onClick={() => setRendererWizardOpen(false)} aria-label="Close renderer wizard">
                Close
              </button>
            </header>

            <div className="gis-renderer-wizard-progress" aria-hidden="true">
              <span className={wizardRiskKey ? 'active' : ''}>1. Risk type</span>
              <i />
              <span className={wizardMeasure ? 'active' : ''}>2. Value kind</span>
              <i />
              <span className={wizardSelectedRenderer ? 'active' : ''}>3. Apply</span>
            </div>

            <div className="gis-renderer-wizard-grid">
              <section>
                <h3>Choose Risk Type</h3>
                <div className="gis-renderer-option-list">
                  {wizardRiskOptions.map(([key, renderer]) => (
                    <button
                      className={wizardRiskKey === key ? 'active' : ''}
                      key={key}
                      type="button"
                      onClick={() => selectWizardRisk(key)}
                    >
                      <strong>{rendererRiskTypeLabel(renderer)}</strong>
                      <span>{rendererRiskTypeDescription(renderer)}</span>
                    </button>
                  ))}
                </div>
              </section>
              ...
              <section>
                <h3>Choose Value Kind</h3>
                <div className="gis-renderer-measure-grid">
                  {wizardMeasureOptions.map(([measure]) => (
                    <button
                      className={wizardMeasure === measure ? 'active' : ''}
                      key={measure}
                      type="button"
                      onClick={() => setWizardMeasure(measure)}
                    >
                      {measure}
                    </button>
                  ))}
                </div>
                <div className="gis-renderer-preview">
                  <span>Preview</span>
                  <strong>
                    {wizardSelectedRenderer
                      ? `${rendererRiskTypeLabel(wizardSelectedRenderer)} using ${rendererMeasureLabel(wizardSelectedRenderer)} values`
                      : 'Select a risk type and value kind'}
                  </strong>
                </div>
              </section>
            </div>
            ...
            <footer className="gis-renderer-modal-footer">
              <button type="button" onClick={() => setRendererWizardOpen(false)}>
                Cancel
              </button>
              <button type="button" onClick={applyRendererWizard} disabled={!wizardSelectedRenderer}>
                Apply Renderer
              </button>
            </footer>
          </section>
