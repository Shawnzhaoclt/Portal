...
export default function App() {
  ...
  const openSelectedFeatureStreetView = useCallback(() => {
    ...
    void openExternalUrl(selectedFeatureStreetViewUrl).catch((error) => {
  ...
  const showError = (error: Error) => {
    console.error(error);
    toast.error("The map could not load a required resource.", {
      description: error.message || "MapLibre reported an unknown error.",
    });
  };
...
function googleStreetViewUrl(point: LngLatPair): string {
  const [lng, lat] = point;
  return GOOGLE_STREET_VIEW_URL_TEMPLATE
    .replace("{lat}", lat.toFixed(7))
    .replace("{lng}", lng.toFixed(7));
}
