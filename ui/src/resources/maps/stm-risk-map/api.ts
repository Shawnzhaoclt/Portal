...
export async function fetchMapStyle(stylePath: string): Promise<{ style: MapStyle; styleBase: URL }> {
  ...
  style.glyphs = clientSetting("externalServices", "maps", "glyphsUrl") || style.glyphs;
...
function apiOrigin(): string {
  return typeof window === "undefined" ? "" : portalDataOrigin();
}
...
function rewriteSpriteUrl(style: MapStyle, styleBase: URL): void {
  ...
  const resolved = new URL(sprite, styleBase);
  style.sprite = apiOrigin()
    ? `${apiOrigin()}${resolved.pathname}${resolved.search}${resolved.hash}`
    : resolved.href;
}
